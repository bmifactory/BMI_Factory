...
#define Myo_TH_For     100 
#define Myo_TH_Rev     200
#define Myo_TH_Halt    100 
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  myo_value = msg.toInt();  
  if (myo_value < Myo_TH_Rev) { //(Myo_TH_For < myo_value) && (
     analogWrite(DC_PWMA, 300);
     digitalWrite(DC_AIN2, HIGH); 
     digitalWrite(DC_AIN1, LOW); 
     digitalWrite(DC_STBY, HIGH); 
  } else if ((Myo_TH_Rev < myo_value)){
     analogWrite(DC_PWMA, 300);
